    ...
    return(
      <Grid container spacing={8}>
        {props.gainers.map((stock, ind) => (
          <Grid key={ind}
                style={{ width: '8%', minHeight: '170px', margin: '1%', padding: '0', position: 'relative', backgroundColor: '#FAFAFA' }}
                item xs
                >
            <div style={{ padding: '10px', paddingTop: '5px' }}>
              <div style={{ fontWeight: 'bold', fontSize: '20px' }} onClick={props.handleClick}>{stock.symbol}</div>
        ...
        <span style={{ fontSize: '10px', marginLeft: '1%' }}>Last update: {props.gainers[0].date}</span>
      </Grid>
    );
    ...
    return(
      <Grid container spacing={8}>
        {props.losers.map((stock, ind) => (
          <Grid key={ind} style={{ width: '8%', minHeight: '170px', margin: '1%', padding: '0', position: 'relative', backgroundColor: '#FAFAFA' }} item xs>
            <div style={{ padding: '10px', paddingTop: '5px' }}>
              <div style={{ fontWeight: 'bold', fontSize: '20px' }} onClick={props.handleClick}>{stock.symbol}</div>
              <div style={{ fontSize: '10px', wordWrap: 'break-word' }}>{stock.companyName}</div>
            </div>
        ...
        <span style={{ fontSize: '10px', marginLeft: '1%' }}>Last update: {props.losers[0].date}</span>
      </Grid>
    );
  } else {
